...
export const getMonthlyData = async (year = new Date().getFullYear()) => {
  try {
    ...
    if (result.success && result.data && Array.isArray(result.data)) {
      ...
      // API returns: { month: "YYYY-MM", totalCollected: amount, transactionCount: count }
      const monthlyMap = new Map();
      
      result.data.forEach(item => {
        const monthStr = item.month || item._id; // Format: "YYYY-MM" or "YYYY-MM"
        if (monthStr) {
          ...
          monthlyMap.set(monthNum, {
            ...
            collected: item.totalCollected || item.total || 0,
 ...
 * Data comes from fee payments table where status = 'pending' or 'partial'
 */
  ...
  try {
    ...
    if (result.success && result.data && Array.isArray(result.data)) {
      return result.data.map((fee) => ({
        id: fee.id,
        studentName: `${fee.student?.firstName || 'Unknown'} ${fee.student?.lastName || ''}`.trim(),
        class: fee.feeStructure?.academicYear || 'N/A',
        amount: parseFloat(fee.amountPending) || 0,
        dueDate: new Date(fee.dueDate).toISOString().split('T')[0],
        status: fee.paymentStatus,
        totalAmount: parseFloat(fee.totalAmount),
        amountPaid: parseFloat(fee.amountPaid),
      }));
    }
